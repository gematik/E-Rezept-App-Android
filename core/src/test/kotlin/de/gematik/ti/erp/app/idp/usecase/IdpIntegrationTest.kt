...
class IdpIntegrationTest {
    ...
    private lateinit var idpRepository: DefaultIdpRepository
    ...
    fun `authenticate with health card`() = runTest {
        ...
        assertEquals(
            true,
            idpRepository.decryptedAccessToken(profileId).first()?.accessToken?.isNotEmpty()
        )
    }
    ...
    fun `authenticate with key store and get paired devices`() = runTest {
        ...
        useCase.pairSecureElementWithHealthCard(
            profileId = profileId,
            cardAccessNumber = cardAccessNumber,
        ...
        useCase.authenticateWithSecureElement(
            profileId = profileId,
