...
@Stable
class Authenticator(
    val authenticatorBiometric: BiometricPromptAuthenticator,
    ...
    fun getAuthResult(profileId: ProfileIdentifier): Flow<PromptAuthenticator.AuthResult> =
        flow {
        ...
        flow {
            emitAll(
                when (bridge.authenticateFor(profileId)) {
                    ...
                    is Biometric ->
                        authenticatorBiometric.authenticate(profileId, PromptAuthenticator.AuthScope.PairedDevices)
    ...
    suspend fun cancelAllAuthentications() {
        authenticatorBiometric.cancelAuthentication()
...
fun rememberAuthenticator(intentHandler: IntentHandler): Authenticator {
    ...
    val promptSE = rememberBiometricPromptAuthenticator(bridge)
    ...
    return remember {
        Authenticator(
            authenticatorBiometric = promptSE,
