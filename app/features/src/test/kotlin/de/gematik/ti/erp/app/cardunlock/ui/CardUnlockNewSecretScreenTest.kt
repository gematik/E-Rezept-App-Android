...
import de.gematik.ti.erp.app.cardunlock.ui.screens.CardUnlockNewSecretScreenPreview
...
class CardUnlockNewSecretScreenTest(config: ScreenshotConfig) : BaseScreenshotTest(config) {

    @Test
    fun screenShotTest() {
        val parameters = CardUnlockNewSecretScreenPreviewDataProvider().values.toList()
        parameters.forEachIndexed { index, parameter ->
            paparazzi.snapshot("parameter_$index") {
                CardUnlockNewSecretScreenPreview(parameter)
            }
        }
    }
}
