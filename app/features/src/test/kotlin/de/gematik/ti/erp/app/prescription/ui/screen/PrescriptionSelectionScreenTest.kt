...
class PrescriptionSelectionScreenTest(config: ScreenshotConfig) : BaseScreenshotTest(config) {
    @Test
    fun screenShotTest() {
        val prescriptionSelectionPreviews = PrescriptionSelectionPreviewParameter().values.toList()
        prescriptionSelectionPreviews.forEachIndexed { index, previewData ->
            paparazzi.snapshot("parameter_$index ${previewData.name}") {
                PrescriptionSelectionScreenPreview(previewData)
            }
        }
    }
}
