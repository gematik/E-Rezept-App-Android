...
class EuConsentScreenTest(config: ScreenshotConfig) : BaseScreenshotTest(config) {
    ...
    @Test
    fun screenShotTest() {
        val parameters = EuConsentPreviewParameterProvider().values.toList()
        parameters.forEach { previewData ->
            paparazzi.snapshot(previewData.name) {
                EuConsentScreenScaffoldPreview(previewData)
            }
        }
    }
}

class EuConsentScreenAccessibilityTest(config: ScreenshotConfig) : BaseAccessibilityTest(config) {
