...
class EuRedemptionCodeScreenTest(config: ScreenshotConfig) : BaseScreenshotTest(config) {
    @Test
    fun screenShotTest() {
        val testParameters = EuRedemptionCodePreviewParameterProvider().values.toList()
        testParameters.forEach { data ->
            paparazzi.snapshot(data.name) {
                EuRedemptionCodeScreenScaffoldPreview(previewData = data)
            }
        }
    }
}

class EuRedemptionScreenAccessibilityTest(config: ScreenshotConfig) : BaseAccessibilityTest(config) {
    @Test
    fun screenShotTest() {
        val testParameters = EuRedemptionCodePreviewParameterProvider().values.toList()
        testParameters.forEach { data ->
