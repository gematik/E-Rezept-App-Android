...
class EuDeleteAccessCodeBottomSheetScreenTest(config: ScreenshotConfig) : BaseScreenshotTest(config) {
    @Test
    fun screenShotTest() {
        val parameters = EuDeleteAccessCodeBottomSheetContentPreviewProvider().values.toList()
        parameters.forEachIndexed { index, previewData ->
            paparazzi.snapshot("$index") {
                EuDeleteAccessCodeBottomSheetContentPreview(previewData)
            }
        }
    }
}

class EuDeleteAccessCodeBottomSheetScreenAccessibilityTest(config: ScreenshotConfig) : BaseAccessibilityTest(config) {

    @Test
    fun screenShotTest() {
        val parameters = EuDeleteAccessCodeBottomSheetContentPreviewProvider().values.toList()
        parameters.forEach { previewData ->
            paparazzi.accessibilitySnapshot(previewData.name) {
                EuDeleteAccessCodeBottomSheetContentPreview(previewData)
            }
        }
    }
