...
class ScheduleDateRangeScreenTest(config: ScreenshotConfig) : BaseScreenshotTest(
    config = config,
    renderingMode = SessionParams.RenderingMode.SHRINK
) {
    @Test
    fun screenShotTest() {
        val scheduleDateRangeScreenPreviews = ScheduleDateRangeScreenPreviewParameter().values.toList()
        scheduleDateRangeScreenPreviews.forEach { parameter ->
            paparazzi.snapshot(parameter.name) {
                ScheduleDateRangeScreenPreview(parameter)
            }
        }
    }
}
