...
class OnboardingPasswordAuthenticationScreenTest(config: ScreenshotConfig) : BaseScreenshotTest(config) {
    ...
    @Test
    fun screenShotTest() {
        val testParameters = AuthScenarioPreviewParameterProvider().values.toList()
        testParameters.forEach {
            paparazzi.snapshot(it.name) {
                OnboardingSelectAppLoginPreview(it)
            }
        }
    }
}

class OnboardingPasswordAuthenticationScreenAccessibilityTest(config: ScreenshotConfig) : BaseAccessibilityTest(config) {
    @Test
    fun screenShotTest() {
        val testParameters = AuthScenarioPreviewParameterProvider().values.toList()
        testParameters.forEach {
