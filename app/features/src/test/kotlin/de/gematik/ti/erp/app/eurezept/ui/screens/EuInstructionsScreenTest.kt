...
class EuInstructionsScreenTest(config: ScreenshotConfig) : BaseScreenshotTest(config) {
    @Test
    fun screenShotTest() {
        val testParameters = EuInstructionsPreviewParameterProvider().values.toList()
        testParameters.forEach { data ->
            paparazzi.snapshot(data.name) {
                EuInstructionsScreenPreview(data)
            }
        }
    }
}

class EuInstructionsScreenAccessibilityTest(config: ScreenshotConfig) : BaseAccessibilityTest(config) {
    @Test
    fun screenShotTest() {
        val testParameters = EuInstructionsPreviewParameterProvider().values.toList()
        testParameters.forEach { data ->
