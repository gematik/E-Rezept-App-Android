...
class InsecureDeviceScreenTest(config: ScreenshotConfig) : BaseScreenshotTest(config) {

    @Test
    fun screenShotTest() {
        BooleanPreviewParameterProvider().values.forEach { checked ->
            paparazzi.snapshot("checked_$checked") {
                InsecureDeviceScreenPreview(checked = checked)
            }
        }
    }
}
