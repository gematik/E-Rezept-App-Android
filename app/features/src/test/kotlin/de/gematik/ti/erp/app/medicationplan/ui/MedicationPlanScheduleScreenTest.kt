...
class MedicationPlanScheduleScreenTest(config: ScreenshotConfig) : BaseScreenshotTest(
    config = config,
    renderingMode = SessionParams.RenderingMode.SHRINK
) {
    @Test
    fun screenShotTest() {
        val medicationPlanScheduleScreenPreviews = MedicationPlanScheduleScreenPreviewParameter().values.toList()
        medicationPlanScheduleScreenPreviews.forEach { parameter ->
            paparazzi.snapshot(parameter.name) {
                MedicationPlanScheduleScreenPreview(parameter)
            }
        }
    }
}
