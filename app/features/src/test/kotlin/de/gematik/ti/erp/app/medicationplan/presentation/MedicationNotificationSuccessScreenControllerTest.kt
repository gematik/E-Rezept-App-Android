...
class MedicationNotificationSuccessScreenControllerTest {
    ...
    private lateinit var controllerUnderTest: MedicationNotificationSuccessScreenController
    ...
    fun setUp() {
        ...
        controllerUnderTest = MedicationNotificationSuccessScreenController(
            loadProfilesWithSchedulesUseCase = loadProfilesWithSchedulesUseCase,
            now = now
        )
    }
    ...
    fun `test data state with active structured schedule`() {
        val schedule = PrescriptionData.Synced(task = API_ACTIVE_SYNCED_TASK_STRUCTURED_DOSAGE, now = now)
