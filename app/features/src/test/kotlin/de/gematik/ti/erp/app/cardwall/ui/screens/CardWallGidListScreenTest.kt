...
class CardWallGidListScreenTest(config: ScreenshotConfig) : BaseScreenshotTest(config) {

    @Test
    fun screenShotTest() {
        val parameters = HealthInsuranceDataPreviewParameterProvider().values.toList()
        parameters.forEachIndexed { index, healthInsuranceData ->
            paparazzi.snapshot("$index") {
                GidListScreenScaffoldPreview(healthInsuranceData)
            }
        }
    }
}
