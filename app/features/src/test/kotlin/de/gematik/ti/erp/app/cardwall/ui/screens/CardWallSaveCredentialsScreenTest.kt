...
class CardWallSaveCredentialsScreenTest(config: ScreenshotConfig) : BaseScreenshotTest(config) {

    @Test
    fun screenShotTest() {
        val parameters = CardWallSaveCredentialsPreviewParameterProvider().values.toList()
        parameters.forEachIndexed { index, previewData ->
            paparazzi.snapshot("parameter_$index ${previewData.name}") {
                CardWallSaveCredentialsScreenPreview(previewData)
            }
        }
    }
}
