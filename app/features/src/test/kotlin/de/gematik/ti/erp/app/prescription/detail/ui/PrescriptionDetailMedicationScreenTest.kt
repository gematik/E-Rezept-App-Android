...
class PrescriptionDetailMedicationScreenTest(config: ScreenshotConfig) : BaseScreenshotTest(
    ...
    @Test
    fun screenShotTest() {
        val previewParameters = PrescriptionDetailPreviewParameterProvider().values.toList()
        previewParameters.forEach { data ->
            paparazzi.snapshot(data.name) {
                PrescriptionDetailMedicationScreenPreview(data)
            }
        }
    }
}
