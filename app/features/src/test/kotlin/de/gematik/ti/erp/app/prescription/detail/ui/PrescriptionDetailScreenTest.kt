...
class PrescriptionDetailScreenTest(config: ScreenshotConfig) : BaseScreenshotTest(config) {
    @Test
    fun screenShotTest() {
        val prescriptionDetailPreviews = PrescriptionDetailPreviewParameter().values.toList()
        prescriptionDetailPreviews.forEach { prescriptionDetailPreview ->
            paparazzi.snapshot(prescriptionDetailPreview.name) {
                PrescriptionDetailScreenPreview(prescriptionDetailPreview)
            }
        }
    }
}
