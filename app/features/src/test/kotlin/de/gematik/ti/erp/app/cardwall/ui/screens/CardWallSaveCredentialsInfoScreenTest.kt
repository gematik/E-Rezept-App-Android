...
class CardWallSaveCredentialsInfoScreenTest(config: ScreenshotConfig) : BaseScreenshotTest(config) {

    @Test
    fun screenShotTest() {
        val parameters = CardWallSaveCredentialsInfoPreviewParameterProvider().values.toList()
        parameters.forEachIndexed { index, previewData ->
            paparazzi.snapshot("parameter_$index ${previewData.name}") {
                CardWallSaveCredentialsInfoScreenPreview(previewData)
            }
        }
    }
}
