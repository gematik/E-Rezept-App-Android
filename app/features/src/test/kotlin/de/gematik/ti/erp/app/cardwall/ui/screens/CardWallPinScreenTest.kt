...
class CardWallPinScreenTest(config: ScreenshotConfig) : BaseScreenshotTest(config) {

    @Test
    fun screenShotTest() {
        CardWallPinScreenPreviewParameterProvider().values.forEach { previewData ->
            paparazzi.snapshot(previewData.name) {
                CardWallPinScreenPreview(previewData)
            }
        }
    }
}
