...
class EuRedeemOverviewScreenTest(config: ScreenshotConfig) : BaseScreenshotTest(config) {

    @Test
    fun screenShotTest() {
        val parameters = EuRedeemPreviewParameterProvider().values.toList()
        parameters.forEachIndexed { index, previewData ->
            paparazzi.snapshot("$index") {
                EuRedeemScreenScaffoldPreview(previewData)
            }
        }
    }
}
