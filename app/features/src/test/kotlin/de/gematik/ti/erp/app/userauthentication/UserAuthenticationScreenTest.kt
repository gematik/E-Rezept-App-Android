...
import de.gematik.ti.erp.app.userauthentication.ui.screens.UserAuthenticationScreenPreview
import org.junit.Test

class UserAuthenticationScreenTest(config: ScreenshotConfig) : BaseScreenshotTest(config) {

    @Test
    fun screenShotTest() {
        val parameters = UserAuthenticationPreviewParameterProvider().values.toList()
        parameters.forEach { previewData ->
            paparazzi.snapshot(previewData.name) {
                UserAuthenticationScreenPreview(
