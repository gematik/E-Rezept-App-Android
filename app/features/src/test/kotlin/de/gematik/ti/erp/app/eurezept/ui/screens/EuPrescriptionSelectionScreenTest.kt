...
class EuPrescriptionSelectionScreenTest(config: ScreenshotConfig) : BaseScreenshotTest(config) {

    @Test
    fun screenShotTest() {
        val parameters = EuPrescriptionSelectionPreviewParameterProvider().values.toList()
        parameters.forEachIndexed { index, previewData ->
            paparazzi.snapshot("$index") {
                EuPrescriptionSelectionContentPreview(previewData)
            }
        }
    }
}
