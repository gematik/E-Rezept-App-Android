...
class MedicationListScheduleScreenTest(config: ScreenshotConfig) : BaseScreenshotTest(
    config = config,
    renderingMode = SessionParams.RenderingMode.SHRINK
) {
    @Test
    fun screenShotTest() {
        val medicationListScheduleScreenPreviews = MedicationListScheduleScreenPreviewParameter().values.toList()
        medicationListScheduleScreenPreviews.forEach { parameter ->
            paparazzi.snapshot(parameter.name) {
                MedicationListScheduleScreenPreview(parameter)
            }
        }
    }
}
