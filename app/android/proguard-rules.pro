########################################################################################
# ⚠️ IMPORTANT: DO NOT REMOVE ANY RULES BELOW UNLESS FULLY VERIFIED ⚠️
# These rules are critical to prevent release-time crashes due to class/method removal
# by R8/Proguard. Even if the code compiles, the app may crash **only in release** if
# required classes (used via reflection, serialization, DI, Room, Realm, etc.) are stripped.
#
# All shared models, migration classes, serialization targets, and DI entry points
# MUST be explicitly kept here if used in the release app. Feature module rules may NOT
# be applied automatically unless exported via consumerProguardFiles.
#
# ❗ Keep this section in sync with recent code structure changes.
########################################################################################
...
-keep class de.gematik.ti.erp.app.db.LatestManualMigration { *; } # present in another module
