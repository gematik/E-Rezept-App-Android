...
class MarkAllUnreadMessagesAsReadUseCase(
    ...
    suspend operator fun invoke() {
        withContext(dispatcher) {
            val messages = communicationRepository.getAllUnreadMessages().first()
            val invoices = invoiceRepository.getAllUnreadInvoices().first()

            coroutineScope {
                launch {
                    messages.forEach { message ->
                        communicationRepository.setCommunicationStatus(
                ...
                launch {
                    invoices
                        ...
                        .forEach { invoice ->
                            invoiceRepository.updateInvoiceCommunicationStatus(
                                invoice.taskId,
                                consumed = true
                            )
                        }
