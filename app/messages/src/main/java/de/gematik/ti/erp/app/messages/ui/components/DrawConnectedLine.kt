...
internal fun Modifier.drawConnectedLine(
    ...
    drawBehind {
        ...
        if (drawFilledTop) {
            ...
            // ------------------------------------------
            val animatedTopEnd = textYPosition * lineProgress
            drawFixedLine(
                start = Offset(center.x, 0f),
                end = Offset(center.x, animatedTopEnd),
                lineColor = lineColor
            )
        } else {
            // Dotted section does NOT animate (on purpose)
            drawDottedTop(
                center = center,
        ...
        // ------------------------------------------
        if (drawFilledBottom) {
            val animatedBottomStart = textYPosition + (size.height - textYPosition) * (1f - lineProgress)

            drawFixedLine(
                start = Offset(center.x, animatedBottomStart),
