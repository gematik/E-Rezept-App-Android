...
class MessageDetailControllerTest {
    ...
    fun `when orderId is provided and returns data successfully`() {
        ...
        } returns flowOf(listOf(COMMUNICATION_DATA_WITH_TASK_ID))
        every { communicationRepository.loadDispReqCommunications(ORDER_ID) } returns flowOf(
            listOf(
                COMMUNICATION_DATA_WITH_TASK_ID
            )
        )
        ...
        testScope.runTest {
            ...
            TestCase.assertEquals(expectedMessages, actualMessages)
    ...
    fun `when orderId is provided but messages and order data are not found`() {
        ...
        every { communicationRepository.loadRepliedCommunications(emptyList(), "") } returns flowOf(
            emptyList()
        )
        every { communicationRepository.loadDispReqCommunications(ORDER_ID) } returns flowOf(
            emptyList()
    ...
    fun `fetch inapp messages from local database`() {
        ...
        testScope.runTest {
            ...
            TestCase.assertEquals(messageList, listOf(welcomeMessage.toInAppMessage()))
        }
    }
    ...
    fun `consumeAllMessages updates communication statuses correctly`() {
        ...
        every { communicationRepository.loadAllRepliedCommunications(listOf(TASK_ID)) } returns flowOf(
            listOf(COMMUNICATION_DATA_WITH_TASK_ID)
        )
        every { communicationRepository.loadDispReqCommunications(ORDER_ID) } returns flowOf(
            listOf(
                COMMUNICATION_DATA_WITH_TASK_ID
        ...
        coEvery { internalMessagesRepository.setInternalMessagesAsRead() } returns Unit
        every { communicationRepository.taskIdsByOrder(ORDER_ID) } returns flowOf(listOf(TASK_ID))
        every { communicationRepository.loadRepliedCommunications(any(), any()) } returns flowOf(
            listOf(COMMUNICATION_DATA_WITH_TASK_ID)
        )
        every { communicationRepository.loadSyncedByTaskId(TASK_ID) } returns flowOf(
            MOCK_SYNCED_TASK_DATA_01_NEW
