...
@AndroidEntryPoint
class DebugFragment : ComposeBaseFragment() {

    @Inject
    @NetworkSecureSharedPreferences
    lateinit var securePrefs: SharedPreferences

    val viewModel by viewModels<DebugSettingsViewModel>()

    override fun viewModels(): ViewModelSet {
        return setOf(viewModel)
    }

    override val content = @Composable { DebugSettings(findNavController()) }
}
...
@Composable
fun DebugSettings(navigation: NavController) {

    val (viewModel: DebugSettingsViewModel) = AppModel.viewModels
