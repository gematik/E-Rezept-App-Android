...
abstract class UriNavType<T>(override val isNullableAllowed: Boolean) : NavType<T>(isNullableAllowed) {
...
open class Route(private val path: String, vararg arguments: NamedNavArgument) {
    ...
    fun path(vararg attributes: Pair<String, Any?>): String {
        ...
        return attributes.fold(Uri.Builder().path(path)) { uri, attr ->
            val arg = requireNotNull(arguments.find { it.name == attr.first }) { "`${attr.first}` not specified within arguments." }
            ...
            when (val v = attr.second) {
                ...
                is Parcelable -> {
                    val type = requireNotNull(arg.argument.type as? UriNavType) { "Parcelable types must be accompanied with an `UriNavType` argument." }
