    ...
    NavHost(
        ...
        composable(HealthCardOrderNavigationScreens.HealthCardOrderInsuranceCompanies.route) {
            ...
            NavigationAnimation(mode = navigationMode) {
                AnimatedElevationScaffold(
                    topBarTitle = title,
                    scrollPosition = if (listState.firstVisibleItemIndex > 0) Int.MAX_VALUE else listState.firstVisibleItemScrollOffset,
...
private fun ContactInsurance(
    ...
    if (option == HealthCardOrderViewModelData.ContactInsuranceOption.PinOnly) {
        ContactMethodRow(
            phone = company.pinPhone,
            url = company.pinUrl,
            mail = company.pinMail
        )
    }
}
