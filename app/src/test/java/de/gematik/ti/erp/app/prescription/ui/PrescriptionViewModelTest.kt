...
class PrescriptionViewModelTest {
    ...
    @MockK
    private lateinit var navigationObservable: NavigationObservable
    ...
    private fun instantiateViewModel() {
        viewModel = PrescriptionViewModel(
            ...
            navigationObservable = navigationObservable,
    ...
    fun `if demo mode was already activated  - demo hint is not available anymore`() =
        coroutineRule.testDispatcher.runBlockingTest {
            ...
            assertNull(
                viewModel.screenState().first().hints.find { it == PrescriptionScreenHintTryDemoMode }
            )
        }
    ...
    fun `if demo mode was canceled  - demo hint is not available anymore`() =
        coroutineRule.testDispatcher.runBlockingTest {
            ...
            every { hintUseCase.cancelledHints } returns flowOf(setOf(PrescriptionScreenHintTryDemoMode))
    ...
    fun `if demo mode is active and welcome hint has been canceled  - welcome to demo hint is not available anymore`() =
        coroutineRule.testDispatcher.runBlockingTest {
            ...
            every { demoUseCase.demoModeActive } answers { MutableStateFlow(true) }
            every { hintUseCase.cancelledHints } returns flowOf(setOf(PrescriptionScreenHintDemoModeActivated))

            instantiateViewModel()

            assertNull(
                viewModel.screenState().first().hints.find { it == PrescriptionScreenHintDemoModeActivated }
            )
        }
}
