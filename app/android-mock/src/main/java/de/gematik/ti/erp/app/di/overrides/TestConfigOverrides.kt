...
fun DI.MainBuilder.testScenarioOverrides(scenario: TestScenario) {
    when (scenario.isOnboardingDone) {
        true -> bindProvider<SettingsDataSource>(overrides = true) { OnboardingDoneMockSettingsDataSource() }
        false -> bindProvider<SettingsDataSource>(overrides = true) { OnboardingNotDoneMockSettingsDataSource() }
    }
}
