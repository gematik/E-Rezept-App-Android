...
class ToggleTranslationConsentUseCase(
    ...
    suspend operator fun invoke(state: Boolean) {
        withContext(dispatcher) {
            when (state) {
                true -> repository.enableConsentForLocalTranslation()
                false -> {
                    repository.disableConsentForLocalTranslation()
                    repository.clearTargetLanguageTag()
                }
            }
        }
    }
