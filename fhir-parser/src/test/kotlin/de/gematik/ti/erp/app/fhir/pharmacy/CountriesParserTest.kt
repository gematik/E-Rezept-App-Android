...
import de.gematik.ti.erp.app.data.fhirVzdCountriesBundle
...
class CountriesParserTest {
    ...
    fun `parse fhirVzd bundle for pharmacy bundle`() = runTest {
        val bundle = Json.parseToJsonElement(fhirVzdCountriesBundle)
        val result = parser.extract(bundle)
        assertEquals(3, result?.countries?.size)
        assertEquals(listOf("LI", "SI", "LT"), result?.countries?.map { it.code })
    }
}
