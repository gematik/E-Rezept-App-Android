...
@Reference(
    info = "kbv bundle for version 1.1.2",
    url = "https://simplifier.net/packages/kbv.ita.erp/1.1.2/files/2212799"
)
@Reference(
    info = "kbv bundle for version 1.2.0",
    url = "https://simplifier.net/packages/kbv.ita.erp/1.2.0/files/2777662"
)
@Requirement(
    "O.Source_2#10",
...
class TaskMedicalDataParser : BundleParser {
    ...
    override fun extract(bundle: JsonElement): FhirTaskDataErpModel? {
        return runCatching {
            if (!bundle.containsExpectedProfileVersionForTaskKbvPhase()) {
                Napier.e(tag = "fhir-parser") { "TaskKBVParser: bundle does not contain expected profile version for Task KBV phase" }
                return null
            }
                ...
                when (entry.kbvResourceType()) {
                    ...
                    FhirKbvResourceType.Coverage -> {
                        ...
                        if (areValidVersions) {
                            ...
                            Napier.w(tag = "fhir-parser", message = "Unsupported Coverage version: ${entry.version}")
                        }
                    }
            ...
            Napier.e(tag = "fhir-parser") { "TaskKBVParser: error: ${it.message}" }
        }.getOrNull()
    }
