...
import de.gematik.ti.erp.app.fhir.prescription.model.original.FhirExtensionReduced.Companion.findExtensionByUrl
...
import de.gematik.ti.erp.app.fhir.prescription.model.original.FhirMedicationProfileType.entries
import de.gematik.ti.erp.app.fhir.prescription.model.original.FhirMedicationVersion.entries
...
data class FhirMedication(
    ...
    companion object {
        ...
        private fun JsonElement.isValidMedication(): Boolean = isValidKbvResource(FhirKbvResourceType.Medication)
...
@Serializable
data class FhirMedicationBatch(
...
object FhirMedicationExtensions {
    ...
    fun List<FhirExtension>.findMedicationCategory110() =
        findExtensionByUrl(MEDICATION_CATEGORY)?.valueCoding?.code

    fun List<FhirExtension>.findMedicationCategory102(): String? =
        findExtensionByUrl(MEDICATION_CATEGORY)
            ?.takeIf { it.valueCoding?.system == MEDICATION_CATEGORY_SYSTEM }
            ?.valueCoding?.code

    fun List<FhirExtension>.getVaccine(): Boolean =
        findExtensionByUrl(MEDICATION_VACCINE_EXTENSION)?.valueBoolean == true

    fun List<FhirExtension>.getNormSizeCode(): String? = findExtensionByUrl(MEDICATION_NORM_GROSSE)?.valueCode

    fun List<FhirExtension>.getCompoundingInstructions(): String? =
        findExtensionByUrl(MEDICATION_COMPOUNDING_MANUFACTURING_INSTRUCTIONS)?.valueString

    fun List<FhirExtension>.getCompoundingPackaging(): String? =
...
@Serializable
data class FhirMedicationProfile(
...
@Serializable
data class FhirMedicationAmount(
...
@Serializable
data class FhirMedicationIngredient(
    ...
    companion object {
        ...
        fun FhirMedicationIngredient.toErpModel(
            ...
            amount = strength?.extensions?.findExtensionByUrl(MEDICATION_INGREDIENT_AMOUNT_EXTENSION_URL)?.valueString,
...
@Serializable
data class FhirMedicationAmountNumerator(
...
@Serializable
data class FhirMedicationAmountValueExtension(
