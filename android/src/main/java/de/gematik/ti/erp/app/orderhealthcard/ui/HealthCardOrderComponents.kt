...
fun HealthCardContactOrderScreen(
    ...
    NavHost(
        ...
        composable(HealthCardOrderNavigationScreens.HealthCardOrderInsuranceCompanies.route) {
            ...
            NavigationAnimation(mode = navigationMode) {
                AnimatedElevationScaffold(
                    modifier = Modifier.testTag(TestTag.Settings.InsuranceCompanyList.InsuranceSelectionScreen),
...
private fun HealthInsuranceSelector(
    ...
    LazyColumn(
        state = state,
        modifier = Modifier.fillMaxSize().testTag(TestTag.Settings.InsuranceCompanyList.InsuranceSelectionContent),
...
private fun HealthCardOrder(
    ...
    LazyColumn(
        modifier = Modifier.fillMaxWidth().testTag(TestTag.Settings.OrderEgk.OrderEgkContent),
        state = listState,
        contentPadding = WindowInsets.navigationBars.only(WindowInsetsSides.Bottom).asPaddingValues()
    ) {
        item {
            Column(Modifier.padding(PaddingDefaults.Medium)) {
                ...
                HintTextLearnMoreButton(
                    modifier = Modifier.testTag(TestTag.Settings.OrderEgk.NFCExplanationPageLink).align(Alignment.End),
        ...
        item {
            ...
            TextButton(
                ...
                modifier = Modifier.padding(horizontal = PaddingDefaults.Small)
                    .testTag(TestTag.Settings.OrderEgk.ChooseInsuranceButton)
            ) {
...
private fun NoContactsHint() =
    HintCard(
        modifier = Modifier.padding(horizontal = PaddingDefaults.Medium)
            .testTag(TestTag.Settings.ContactInsuranceCompany.NoContactInfoTextBox),
...
private fun ContactInsuranceOptions(
    ...
    Column {
        Option(
            testTag = TestTag.Settings.ContactInsuranceCompany.OrderEgkAndPinRadioButton,
        ...
        Option(
            testTag = TestTag.Settings.ContactInsuranceCompany.OrderPinRadioButton,
...
private fun ContactMethodRow(
    ...
    Row(
        ...
        phone?.let {
            ContactMethod(
                modifier = Modifier.weight(1f).testTag(TestTag.Settings.ContactInsuranceCompany.TelephoneButton),
        ...
        url?.let {
            ContactMethod(
                modifier = Modifier.weight(1f).testTag(TestTag.Settings.ContactInsuranceCompany.WebsiteButton),
        ...
        mail?.let {
            ...
            ContactMethod(
                modifier = Modifier.weight(1f).testTag(TestTag.Settings.ContactInsuranceCompany.MailToButton),
