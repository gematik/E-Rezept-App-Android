...
fun LazyListScope.pznMedicationInformation(medication: SyncedTaskData.MedicationPZN) {
    item {
        Label(text = medication.text, label = stringResource(R.string.medication_trade_name))
    }
...
fun LazyListScope.compoundingMedicationInformation(
    ...
    item {
        Label(text = medication.text, label = stringResource(R.string.medication_compounding_name))
    }
    medication.amount?.let {
        item {
            AmountLabel(it)
...
fun AuthoredOnLabel(authoredOn: Instant) {
    Label(
        text = remember { dateTimeMediumText(authoredOn) },
...
fun ExpirationDateLabel(expirationDate: Instant) {
    Label(
        text = remember { dateTimeMediumText(expirationDate) },
