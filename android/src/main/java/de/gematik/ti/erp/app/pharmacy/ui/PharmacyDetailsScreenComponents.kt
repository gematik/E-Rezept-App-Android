...
fun PharmacyDetailsScreen(
    ...
    onClickFavoriteStar: (PharmacyUseCaseData.Pharmacy, Boolean) -> Unit
) {
    ...
    AnimatedElevationScaffold(
        modifier = Modifier.testTag(TestTag.PharmacySearch.OrderOptions.Screen),
        topBarTitle = stringResource(R.string.pharmacy_detail_title),
        elevated = scrollState.value > 0,
        actions = {
            state?.let { state ->
                FavoriteStarButton(state.markedAsFavorite) {
                    onClickFavoriteStar(state.selectedPharmacy, it)
                }
            }
        },
...
private fun FavoriteStarButton(
    ...
    var isMarked by remember { mutableStateOf(isMarkedAsFavorite) }
