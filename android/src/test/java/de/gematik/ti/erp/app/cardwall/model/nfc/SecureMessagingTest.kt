...
import de.gematik.ti.erp.app.cardwall.model.nfc.card.PaceKey
import de.gematik.ti.erp.app.cardwall.model.nfc.card.SecureMessaging
...
class SecureMessagingTest {
    private val keyEnc: ByteArray = "68406B4162100563D9C901A6154D2901".decodeHex().toByteArray()
    private val keyMac: ByteArray = "73FF268784F72AF833FDC9464049AFC9".decodeHex().toByteArray()
    ...
    fun testEncryptionCase1() {
        ...
        val expectedEncryptedApdu = "0D0203040A8E08D92B4FDDC2BBED8C00".decodeHex().toByteArray()
    ...
    fun testEncryptionCase2s() {
        ...
        val expectedEncryptedApdu = "0D02030400000D97017F8E0871D8E0418DAE20F30000".decodeHex().toByteArray()
        val encryptedCommandApdu = secureMessaging.encrypt(commandApdu)
        Assert.assertArrayEquals(
            expectedEncryptedApdu,
            encryptedCommandApdu.bytes
    ...
    fun testEncryptionCase2e() {
        ...
        val expectedEncryptedApdu = "0D02030400000E970201018E089F3EDDFBB1D3971D0000".decodeHex().toByteArray()
        val encryptedCommandApdu = secureMessaging.encrypt(commandApdu)
        Assert.assertArrayEquals(
            expectedEncryptedApdu,
            encryptedCommandApdu.bytes
    ...
    fun testEncryptionCase3s() {
        ...
        val expectedEncryptedApdu =
            "0D0203041D871101496C26D36306679609665A385C54DB378E08E7AAD918F260D8EF00".decodeHex().toByteArray()
        val encryptedCommandApdu = secureMessaging.encrypt(commandApdu)
        Assert.assertArrayEquals(
            expectedEncryptedApdu,
            encryptedCommandApdu.bytes
    ...
    fun testEncryptionCase4s() {
        ...
        val expectedEncryptedApdu =
            "0D020304000020871101496C26D36306679609665A385C54DB3797017F8E0863D541F262BD445A0000".decodeHex().toByteArray()
        val encryptedCommandApdu = secureMessaging.encrypt(commandApdu)
        Assert.assertArrayEquals(
            expectedEncryptedApdu,
            encryptedCommandApdu.bytes
    ...
    fun shouldDecryptDo99Apdu() {
        val secureMessaging = SecureMessaging(paceKey)
        val apduToDecrypt = ResponseApdu("990290008E08087631D746F872729000".decodeHex().toByteArray())
    ...
    fun shouldDecryptDo87Apdu() {
        val secureMessaging = SecureMessaging(paceKey)
        val apduToDecrypt =
            ResponseApdu("871101496c26d36306679609665a385c54db37990290008E08B7E9ED2A0C89FB3A9000".decodeHex().toByteArray())
        val decryptedApdu: ResponseApdu = secureMessaging.decrypt(apduToDecrypt)
        val expectedDecryptedApdu = ResponseApdu("05060708090a9000".decodeHex().toByteArray())
        Assert.assertArrayEquals(
            expectedDecryptedApdu.bytes,
            decryptedApdu.bytes
        )
    ...
    fun decryptShouldFailWithMissingStatusBytes() {
        val secureMessaging = SecureMessaging(paceKey)
        val apduToDecrypt =
            ResponseApdu("871101496c26d36306679609665a385c54db378E08B7E9ED2A0C89FB3A9000".decodeHex().toByteArray())
        try {
    ...
    fun decryptShouldFailWithMissingStatus() {
        val secureMessaging = SecureMessaging(paceKey)
        val apduToDecrypt =
            ResponseApdu("871101496c26d36306679609665a385c54db37990290008E08B7E9ED2A0C89FB3A".decodeHex().toByteArray())
        try {
            secureMessaging.decrypt(apduToDecrypt)
    ...
    fun decryptShouldFailWithWrongCCS() {
        val secureMessaging = SecureMessaging(paceKey)
        val apduToDecrypt =
            ResponseApdu("871101496c26d36306679609665a385c54db37990290008E08A7E9ED2A0C89FB3A9000".decodeHex().toByteArray())
        try {
            secureMessaging.decrypt(apduToDecrypt)
    ...
    fun decryptShouldFailWithMissingCCS() {
        val secureMessaging = SecureMessaging(paceKey)
        val apduToDecrypt =
            ResponseApdu("871101496c26d36306679609665a385c54db37990290009000".decodeHex().toByteArray())
        try {
            secureMessaging.decrypt(apduToDecrypt)
